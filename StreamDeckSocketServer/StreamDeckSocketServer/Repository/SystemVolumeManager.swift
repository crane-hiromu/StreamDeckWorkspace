...
final class SystemVolumeManager {
    ...
    func adjustVolume(by delta: Float) {
        ...
        if let prev = previousVolume {
            ...
            setVolume(newVolume)
            previousVolume = nil
            ServerMessageSender.shared.sendSystemVolumeChange(volume: Int(newVolume * 100))
    ...
    func toggleMute() {
        if let current = getVolume(), current > 0 {
            ...
            setVolume(0.0)
            ServerMessageSender.shared.sendSystemVolumeChange(volume: Int(0))
        } else if let prev = previousVolume {
            // 前の音量に戻す
            setVolume(prev)
            previousVolume = nil
            ServerMessageSender.shared.sendSystemVolumeChange(volume: Int(prev * 100))
        }
    }
